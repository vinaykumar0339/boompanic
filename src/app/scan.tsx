...
export default function ScanRoute() { const { kind } = useLocalSearchParams<{ kind: 'local' | 'offer' }>(); const [permission, requestPermission] = useCameraPermissions(); const session = useGameSession(); useEffect(() => { if (!permission?.granted) void requestPermission(); }, [permission, requestPermission]); const scanned = ({ data }: { data: string }) => { if (kind === 'local') { void session.joinLocal(data); router.replace('/local/join'); } else { void session.createOnlineAnswer(data); router.replace('/online/join'); } }; if (!permission?.granted) return <View style={styles.empty}><Text style={styles.text}>Camera permission is required to scan a game code.</Text><SecondaryButton label="BACK" onPress={() => router.back()} /></View>; return <View style={styles.camera}><CameraView style={StyleSheet.absoluteFill} facing="back" barcodeScannerSettings={{ barcodeTypes: ['qr'] }} onBarcodeScanned={scanned} /><View style={styles.overlay}><Text style={styles.text}>Scan BoomPanic QR</Text><SecondaryButton label="CANCEL" onPress={() => router.back()} /></View></View>; }
